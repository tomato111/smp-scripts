...
String.prototype.replaceEach = function () { //e.g. 'abc*Bc'.replaceEach('b', 'e', 'c', 'f', '\\*', '_', 'ig')  /* aef_ef */
    ...
    const flag = arguments[arguments.length - 1];
    if (!(arguments.length % 2))
        throw new Error('Wrong number of arguments');

    for (let i = 0; i < arguments.length - 1; i += 2) {
        const re = new RegExp(arguments[i], flag);
...
String.prototype.toHalfWidthNum = function () { //-- Replace full-width number with half-width number --
    return this.replace(/[０１２３４５６７８９]/g, (s) => '０１２３４５６７８９'.indexOf(s));
};

String.prototype.hexNumRefToString = function () { //-- 数値文字参照(16進数, 10進数)を文字列に変換 --
    return this.replace(/&#x([0-9a-f]+);/ig, (match, $1) => String.fromCodePoint('0x' + $1));
};

String.prototype.decNumRefToString = function () {
    return this.replace(/&#(\d+);/ig, (match, $1) => String.fromCodePoint($1));
};

String.prototype.stringTodecNumRef = function () {
    return [...this].reduce((acc, curr) => acc + `&#${curr.codePointAt(0)};`, '');
};

String.prototype.decodeHTMLEntities = function () {
    let str = this.valueOf(); // ここのthisはラッパーオブジェクト
    const entitiesRE = /&\w+;|&#\d+;|&#x[0-9a-f]+;/ig;
    let result; // エンティティだけ抜き出す（改行やhtmlタグ関係を避ける）

    const document = new ActiveXObject('htmlfile');
    document.open();
...
Function.prototype.timeout = function (time, ...args) {
    this.clearTimeout();
    this.$$timeout_timerid$$ = window.SetTimeout(() => {
        this(...args);
    }, time);
};
...
Function.prototype.clearTimeout = function () {
    window.ClearTimeout(this.$$timeout_timerid$$);
};
...
Message.prototype.console = function (s) {
    console.log(this.text + (s ? s : ''));
};
...
TraceLog.prototype = {
    start: function (message) {
        ...
        console.log('[' + message + '] has started');
    },
    stop: function () {
        const current = new Date().getTime();
        const endTime = current - this.startTime;
        console.log('[' + this.message + '] has finished at ' + endTime + ' ms');
    }
};
...
function BinaryStream() {
    ...
    this.readAsHexString = function (size) {
        ...
        const bin = this.readAsBinary(size);
    ...
    this.readAsIntArray = function (size, option = [], isLittleEndian) {
        const arr = [];
        const s = this.readAsHexString(size);
        for (let i = 0; i < s.length;) {
            const sz = (option.length > 0) ? option.shift() * 2 : 2;
            const st = s.substr(i, sz).match(/(\w{2})/g);
    ...
    this.writeFromIntArray = function (arr) {
        ...
        for (let i = 0; i < arr.length; i++) {
            stm.WriteText(String.fromCodePoint(arr[i] & 0xFF));
        }
        this.position = stm.Position;
    };
...
//-- ini file reader --
function Ini(filename, charset) {
    this.open(filename, charset);
}
Ini.prototype = {
    // 初期化
    clear: function () {
    ...
    open: function (filename, charset) {
        ...
        try {
            ...
            while (!stm.EOS) {
                const line = stm.ReadText(-2).trim();
                ...
                if (/^\[(.+)\]\s*$/.test(line)) { // セクション行
                    section = RegExp.$1;
                    this.items.set(section, new Map());
                }
                else if (typeof section !== 'undefined' && /(.+?)=(.*)/.test(line)) { // keyとvalue
                    this.items.get(section).set(RegExp.$1.trim(), RegExp.$2.trim());
                }
            }
    ...
    update: function (filename, charset) {
        ...
        try {
            ...
            for (const [section, map] of this.items) {
                stm.WriteText('[' + section + ']\r\n');
                for (const [key, value] of map)
                    stm.WriteText(`${key}=${value}\r\n`);
                stm.WriteText('\r\n');
            }
    ...
    setItem: function (section, key, value, update) {
        ...
        if (!this.items.has(section))
            this.items.set(section, new Map());

        this.items.get(section).set(key, value);
...
function prompt(text, title, defaultText) {
    const sc = new ActiveXObject('ScriptControl');
    const code = `
    Function fn(text, title, defaultText)
...
function execCommand(path) {
    ...
    if (/(?:\\|:\/\/|\.\w{2,4})/.test(path)) {
        let a, arg = null;
        if (/^(".+?") (.*)/.test(path) || /^([^"].+?\.\w{2,4}) (.*)/.test(path)) {
            path = RegExp.$1;
            a = RegExp.$2.charAt(0);
            arg = (a !== '"' && a !== '/') ? `"${RegExp.$2}"` : RegExp.$2;
        }
...
function writeTextFile(text, file, charset = 'UTF-8') {
    const UTF8N = /^UTF-8N$/i.test(charset);
    const stm = new ActiveXObject('ADODB.Stream');
    stm.Open();
    try {
        stm.Type = 2; //adTypeText
...
//-- Image --
function calcImgSize(img, dstW = 1, dstH = 1, strch, kar = true) {
    ...
    if (strch) { // パネルより小さい画像を拡大する
        ...
        if (kar) { // アスペクト比を考慮
            size.width = Math.ceil(srcW * dstH / srcH); // 画像の縦をパネルの高さと仮定し、横幅を計算
            if (size.width > dstW) { // パネル幅を超えるなら、画像の横をパネル幅と仮定し、高さを計算
                size.width = dstW;
...
function rotateImg(img, flip_type) { // In Spider Monkey Panel, if you just img.RotateFlip, for some reason the ApplyMask method will not work
    img = img.Clone(0, 0, img.Width, img.Height);
    img.RotateFlip(flip_type);
    const newimg = gdi.CreateImage(img.Width, img.Height);
    const g = newimg.GetGraphics();
    g.DrawImage(img, 0, 0, img.Width, img.Height, 0, 0, img.Width, img.Height);
...
function sendToRecycleBin(path) {
    const sa = new ActiveXObject('Shell.Application');
    const recycle_bin_folder = sa.NameSpace(10);
    recycle_bin_folder.MoveHere(path);
}

//-- Shuffle for Array--
function shuffleArray(arr, from = 0) { // arrの配列番号from以降をシャッフルする
    let i = arr.length;
    if (i - from < 2) return;
    while (i - from) {
        const j = Math.floor(Math.random() * (i - from)) + Number(from);
        const t = arr[--i];
        arr[i] = arr[j];
        arr[j] = t;
    }
